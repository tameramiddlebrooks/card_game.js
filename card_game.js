...
for(let i=0;i<suits.length;i++){
  // inner forloop iterate through ranks
  for(let j=0;j<ranks.length;j++){
  // create a card object with suit, rank, value
  let card = {
  "suit": suits[i],
  "rank": ranks[j],
  "value": j,
  ...
  // step 6,return deck array
  return deck;
  }
  // call buildDeck function and store value in deck which is returned from function
// print deck
...
function greet () {
  const name = getInput("What is your name?: ")
  console.log(name);
}
...
function guess(card1, card2) {
  console.log(`${card1.rank} of ${card1.suit}`);
  ...
  if (input === "h") {
    ...
    console.log('h or l has to be picked, no points for this round')
    return false;
  }
...
function playGame () {
  ...
  while (score < 5 && deck.length > 0) {
    ...
    if (guess(currentCard, nextCard) === true) {
      ...
      console.log('You were incorrent, no points were rewarded')
